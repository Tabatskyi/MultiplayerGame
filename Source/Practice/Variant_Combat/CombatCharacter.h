...
protected:
	...
	/**
	 *  Records a per-frame snapshot of head + torso hitbox transforms on the server.
	 *  Maintained server-side only; clients never write to it.
	 */
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category="Lag Compensation",
	          meta=(AllowPrivateAccess="true"))
	ULagCompensationComponent* LagCompComp;

	/**
	 *  The weapon currently equipped by this character.
	 *  Assigned in Blueprint defaults or at runtime by a pickup/spawn system.
	 */
	UPROPERTY(VisibleAnywhere, BlueprintReadWrite, Category="Lag Compensation")
	ALagCompensatedWeapon* EquippedWeapon = nullptr;
...
protected:
	...
	/** Called when the fire weapon input is triggered */
